...
const REDACTION_PATTERNS = [
    ...
    {
        pattern: /\b(api[_\-]?key|api[_\-]?secret|access[_\-]?token|auth[_\-]?token)\s*[:=]\s*['"]?[A-Za-z0-9\-_]{20,}/gi,
...
async function proxyRoutes(fastify) {
    fastify.post("/v1/chat/completions", async (request, reply) => {
        ...
        const sessionId = request.headers["x-session-id"] ?? (0, uuid_1.v4)();
        ...
        const rateCheck = await (0, rateLimiter_1.checkUserRateLimit)(fastify, userId);
        if (!rateCheck.allowed) {
            // Log to audit_events
            try {
                await (0, auditLogger_1.logAuditEvent)(fastify.db, {
        ...
        request.log.info({ requestId, userId, model, promptLength: promptText.length }, "Intercepted request");
        ...
        if (riskResult) {
            ...
            riskResult = await (0, riskEngine_1.analyzePrompt)(requestId, promptText, userId, sessionId);
        ...
        const decision = policyEngineInstance_1.sharedPolicyEngine.evaluate(intercepted);
        request.log.info({ requestId, action: decision.action, rule: decision.rule_name }, "Policy decision made");
        // ── Step 3b: Publish risk scored event ───────────────────────────
        await fastify.kafka.publish(TOPICS.RISK_SCORED, {
            eventId: requestId,
            userId,
        ...
        const anomaly = await (0, anomalyDetector_1.checkAnomalies)(fastify, userId, riskResult.score);
        if (anomaly.isAnomaly) {
            await fastify.kafka.publish(TOPICS.ANOMALIES_DETECTED, {
                eventId: requestId,
                userId,
        ...
        if (decision.action === "block") {
            await fastify.kafka.publish(TOPICS.POLICY_VIOLATIONS, {
                eventId: requestId,
                userId,
        ...
        if (decision.action === "redact") {
            forwardMessages = body.messages.map((msg) => ({
                ...msg,
                content: msg.role === "user" || msg.role === "system"
        ...
        await fastify.kafka.publish(TOPICS.REQUESTS_INBOUND, {
            eventId: requestId,
            userId,
            model,
        ...
        try {
            const { fullText, latencyMs } = await (0, groqClient_1.forwardToGroq)(forwardRequest, reply, shouldStream);
            // Publish outbound response event
            await fastify.kafka.publish(TOPICS.RESPONSES_OUTBOUND, {
                eventId: requestId,
                userId,
                model,
            ...
            request.log.info({ requestId, action: decision.action, latencyMs }, "Request completed");
        }
        catch (err) {
    ...
    fastify.get("/v1/audit/events", async (request, reply) => {
        ...
        const result = await fastify.db.query(`SELECT * FROM audit_events ORDER BY timestamp DESC LIMIT $1 OFFSET $2`, [limit, offset]);
        return reply.send({
            events: result.rows,
            limit,
            offset,
