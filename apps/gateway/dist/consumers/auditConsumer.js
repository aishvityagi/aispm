...
async function startConsumer() {
    ...
    await (0, elasticsearch_1.ensureIndex)();
    ...
    await consumer.run({
        eachMessage: async ({ topic, message }) => {
            ...
            try {
                await pool.query(`INSERT INTO kafka_events (topic, payload) VALUES ($1, $2)`, [topic, JSON.stringify(payload)]);
            ...
            if ((topic === "ai.risk.scored" || topic === "ai.responses.outbound") &&
                payload.eventId) {
                try {
                    ...
                    for (let attempt = 1; attempt <= 5; attempt++) {
                        ...
                        const result = await pool.query(`SELECT * FROM audit_events WHERE request_id = $1 LIMIT 1`, [payload.eventId]);
                    ...
                    if (row) {
                        const r = row;
                        await (0, elasticsearch_1.indexAuditEvent)({
                            id: row.id,
                            user_id: row.user_id,
                        ...
                        console.warn(`[AuditConsumer] No audit_events row found for eventId: ${payload.eventId}`);
                    }
                }
                catch (err) {
...
//# sourceMappingURL=auditConsumer.js.map