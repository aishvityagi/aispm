  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        let result;
        try {
          result = await (fastify as any).db.query(
            `INSERT INTO policy_rules
               (id, name, condition_field, operator, value, action, created_at)
             VALUES ($1, $2, $3, $4, $5, $6, NOW())
             RETURNING *`,
            [id, name, condition_field, operator, value, action]
          );
        } catch (insertErr: any) {
        ...
        const allRules = await (fastify as any).db.query(
          "SELECT * FROM policy_rules ORDER BY created_at ASC"
        );
        sharedPolicyEngine.loadFromDB(allRules.rows);
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        sharedPolicyEngine.removeRule(id);
