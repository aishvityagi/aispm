...
async function kafkaPlugin(fastify: any, options: any): Promise<void> {
  ...
  async function publish(topic: TopicName, payload: Record<string, unknown>): Promise<void> {
    try {
      await producer.send({
        topic,
        messages: [
  ...
  fastify.decorate("kafka", { publish, TOPICS });
